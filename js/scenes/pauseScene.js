export default class Pause extends Phaser.Scene {
    ...
    create(){
        ...
        const botoContinuar = this.add.text(this.canvasWidth/2 - 170, 200, "Resume", { fontSize: "100px", fontFamily: "MyFont", fill: '#000', backgroundColor: '#3c86e7', textPadding: {x: 0, y:0}, fixedWidth: 300, fixedHeight: 100, align: 'center' });
        ...
        const botoGuardar = this.add.text(this.canvasWidth/2 - 170, 325, "Save", { fontSize: "100px", fontFamily: "MyFont", fill: '#000', backgroundColor: '#3c86e7', textPadding: {x: 0, y:0}, fixedWidth: 300, fixedHeight: 100, align: 'center' });
        botoGuardar.setInteractive();
        botoGuardar.on('pointerdown', () => {
            console.log(this.game.config.current);
            console.log(JSON.stringify({"lives":this.game.config.current.livesStart,"bears":this.game.config.current.bearsStart,"escena":this.game.config.current.scene.key}));
            localStorage.setItem("partida",JSON.stringify({"lives":this.game.config.current.livesStart,"bears":this.game.config.current.bearsStart,"escena":this.game.config.current.scene.key}))
